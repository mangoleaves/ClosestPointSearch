...
namespace ClosestPointSearch
{
    ...
    class Add_decorated_point :public Kernel
    {

        class Decorated_point : public Kernel::Point_3
        {
        public:
            const TriIter& id() const { return m_id; }
            ...
            Decorated_point(const typename Point_3& p)
                : Kernel::Point_3(p)
                , m_id()
                , m_is_id_initialized(false) {}
    ...
    typedef CGAL::Search_traits_3<Add_decorated_point> Search_tree_traits;
    typedef CGAL::Orthogonal_k_neighbor_search<Search_tree_traits> Neighbor_search;
    ...
    {
    private:
        Neighbor_search_tree kdtree;
    public:
        KdTree(const std::vector<Vec3d>& points, const std::vector<TriIter>& triIters)
        {
            assert(points.size() == triIters.size());

            std::vector<Add_decorated_point::Point_3> pointIds;
            pointIds.reserve(points.size());
            for (int i = 0; i < points.size(); i++)
            {
                Point_3 tmp(points[i].x, points[i].y, points[i].z);
                pointIds.emplace_back(tmp, triIters[i]);
            }

            kdtree.insert(pointIds.begin(), pointIds.end());
            kdtree.build();
        }

        std::pair<Vec3d, TriIter> search_nearest_point(const Vec3d& query)
        {
            Point_3 tmp(query.x, query.y, query.z);
            Neighbor_search search(kdtree, tmp, 1);

            return std::pair<Vec3d, TriIter>(
                Vec3d(search.begin()->first[0], search.begin()->first[1], search.begin()->first[2]),
                search.begin()->first.id()
                );
        }
    };
#endif 
    ...
    public:
        ...
        double distance_to_point(const KdVec3d& query, KdVec3d& dists)
        {
            ...
            T1 = _mm256_sub_pd(minBound, Q);	
            T2 = _mm256_max_pd(T1, FourZero);	
            T3 = _mm256_mul_pd(T2, T2);			

            T1 = _mm256_sub_pd(Q, maxBound);	
            T4 = _mm256_max_pd(T1, FourZero);	
            T5 = _mm256_fmadd_pd(T4, T4, T3);	
